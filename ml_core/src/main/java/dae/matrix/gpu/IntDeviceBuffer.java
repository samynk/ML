 ...
 */
public class IntDeviceBuffer {

    private final intmatrix cpuBuffer;
    ...
    public IntDeviceBuffer(intmatrix cpuBuffer) {
    ...
    private void initPaddedMem() {
        ...
        deviceOffset[0] = zp * Sizeof.cl_int;
        deviceOffset[1] = zp;
        deviceOffset[2] = 0;
    ...
    protected int getDeviceRowPitch() {
        return deviceDimension[0] * Sizeof.cl_int;
    }

    protected int getDeviceSlicePitch() {
        return deviceDimension[0] * deviceDimension[1] * Sizeof.cl_int;
    }

    protected int getHostRowPitch() {
        return cpuBuffer.getNrOfRows() * Sizeof.cl_int;
    }

    protected int getHostSlicePitch() {
        return cpuBuffer.getSliceSize() * Sizeof.cl_int;
    }
    ...
    public cl_mem getRMem() {
        if (rMemPadded == null) {
            rMemPadded = IntMatrixOpGpu.createMem(cpuBuffer, padding, CL.CL_MEM_READ_ONLY);
        }
        return rMemPadded;
    }
    ...
    public cl_mem getRWMem() {
        if (rwMemPadded == null) {
            rwMemPadded = IntMatrixOpGpu.createMem(cpuBuffer, padding, CL.CL_MEM_READ_WRITE);
        }
        return rwMemPadded;
    }
