...
public class float2 {
    ...
    /**
     * Calculates the cross product of two float2 objects. Because the coordinates
     * are xy coordinates, the cross product will produces a float3 object.
     * @param op1 the first float2 object.
     * @param op2 the second float2 object.
     * @return the cross product of op1 and op2.
     */
    public static float3 Cross(float2 op1, float2 op2){
        float zr = op1.x * op2.y - op1.y * op2.x;
        return new float3(0,0, zr);
    }
